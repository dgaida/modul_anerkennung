...
1. Navigieren Sie zum Tab **Anwendung** (Application) oder **Speicher** (Storage).
1. Wählen Sie in der linken Seitenleiste unter **Cookies** die Adresse `https://module.gm.th-koeln.de` aus.
1. Suchen Sie in der Liste nach dem Cookie mit dem Namen `access_token`.
1. Kopieren Sie den Wert dieses Cookies und tragen Sie ihn als `MOCOGI_API_TOKEN` in Ihre Konfiguration ein.
